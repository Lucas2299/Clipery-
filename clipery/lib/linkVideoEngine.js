...
async function faceCenterX(source, start, end) {
  try {
    ...
    if (r && r.ok && typeof r.x === "number" && r.x >= 0.12 && r.x <= 0.88) {
      console.log(`[face-follow][linkVideoEngine] speaker face at x=${r.x.toFixed(2)} (${r.faces || 0} faces)`);
      return r.x;
    }
